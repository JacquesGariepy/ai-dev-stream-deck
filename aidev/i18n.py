...
STRINGS = {
    ...
    'catalog_loading': ('Detecting local items…', 'Détection des éléments locaux…'),
    'catalog_empty': ('Nothing was detected. Check the message above or choose another project.', 'Aucun élément détecté. Vérifie le message ci-dessus ou choisis un autre projet.'),
    'catalog_no_match': ('No item matches this filter.', 'Aucun élément ne correspond à ce filtre.'),
    ...
    'workflow_mode': ('AI mission workflow: {workflow}. Enter an English objective, then review before launching.', 'Mission IA : {workflow}. Saisis un objectif en anglais, puis vérifie avant de lancer.'),
    'workflow_test_note': ('This prepares an AI testing mission. To run the project test suite directly, use BUILD / TEST > TEST.', 'Ceci prépare une mission de test avec une IA. Pour lancer directement les tests du projet, utilise BUILD / TEST > TEST.'),
    ...
    'git_push_confirm': ('Push branch {branch} to {destination}?\nCommits ahead: {ahead}\nProject: {project}\n\nThe command runs visibly in a terminal.', 'Pousser la branche {branch} vers {destination} ?\nCommits en avance : {ahead}\nProjet : {project}\n\nLa commande s’exécute visiblement dans un terminal.'),
    'git_branch_name': ('New branch name (created from the current HEAD):', 'Nom de la nouvelle branche (créée depuis HEAD) :'),
    'git_branch_invalid': ('Git rejected this branch name.', 'Git refuse ce nom de branche.'),
    'git_switch_title': ('Switch to a local branch', 'Basculer vers une branche locale'),
    'task_kind_missing': ('No {kind} task was detected in this project. Choose a task explicitly.', 'Aucune tâche {kind} détectée dans ce projet. Choisis une tâche explicitement.'),
